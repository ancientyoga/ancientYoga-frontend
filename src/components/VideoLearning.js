...
import api from '../api'; // ✅ Updated: using custom Axios instance
...
function VideoLearning() {
  ...
  const scrollLeft = (courseId) => {
    const container = document.getElementById(`video-learn-scroll-${courseId}`);
    if (container) container.scrollBy({ left: -300, behavior: 'smooth' });
  };

  const scrollRight = (courseId) => {
    const container = document.getElementById(`video-learn-scroll-${courseId}`);
    if (container) container.scrollBy({ left: 300, behavior: 'smooth' });
  };
  ...
  return (
    <>
      ...
      <div className="container py-5">
        <h2 className="text-center mb-5 fw-bold">📺 Course Sample Videos</h2>
        ...
        {Object.entries(groupedVideos).map(([courseId, courseVideos], idx) => {
          ...
          return (
            <div key={courseId || idx} className="mb-5">
              ...
              <div className="video-learn-wrapper position-relative">
                <button className="video-learn-scroll-btn left" onClick={() => scrollLeft(courseId)}>&#8249;</button>

                <div className="video-learn-container" id={`video-learn-scroll-${courseId}`}>
                  {courseVideos.map((video, index) => {
                    const isUnlocked = isEnrolled || index < 2;

                    return (
                      <div key={video.id} className="video-learn-card">
                        <div className="video-card h-100 shadow-sm rounded overflow-hidden">
                          <div className="position-relative">
                            <img
                              src={video.thumbnail ? `${api.defaults.baseURL}/uploads/${video.thumbnail}` : '/default-thumbnail.jpg'}
                              alt={video.title}
                              className="w-100"
                ...
                <button className="video-learn-scroll-btn right" onClick={() => scrollRight(courseId)}>&#8250;</button>
              </div>
